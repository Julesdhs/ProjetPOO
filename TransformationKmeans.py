...
class TransformationKmeans(TransformationAbstraite):
    ...
    Cette modification est un choix arbitraire pour la clarté du résultat

    Attributs
    ----------
    k : int
        un entier qui définit le nombre de classes

    Méthodes
    -------
    transform()
        applique la méthode des k-means au tableau et renvoie un tableau avec deux colonnes
    ...
    def transform(self,table0):
        ...
        def arrange_ordre(table,listcol):
            ...
            for nomcol in listcol :
                t=extrait_colonne(table,nomcol)
...
#reste de la fonction fit
        table,nbvar=arrange_ordre(table0,self.listcol)
        k=self.k
        ...
        classes=extrait_colonne(table,'Classes')[1]
        ...
        while classes != classes2 and nb<50:
            ...
            for j in range (nb_obs):
                ...
                indice = 0
                for i in range (k):
                    dc=distance(table.contenu[j],centres[i],nbvar)
                    if d >= dc :
                        indice=i
                        d=dc
                table.contenu[j][indclasse] = indice
            calcul_centres(table,centres,k,nbvar,indclasse)
            classes=extrait_colonne(table,'Classes')[1]
